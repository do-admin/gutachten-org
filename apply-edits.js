...
try {
  const componentTextEditor = require("./text-editor/lib/component-text-editor.ts");
...
async function applyEdit(edit) {
  ...
  // Log HTML content detection
  if (edit.contains_html_links) {
    console.log(`   🔗 Contains HTML links: Yes`);
    if (edit.link_metadata) {
      const linkMeta =
        typeof edit.link_metadata === "string"
          ? JSON.parse(edit.link_metadata)
          : edit.link_metadata;
      if (linkMeta.linkCount) {
        console.log(`   📊 Link count: ${linkMeta.linkCount}`);
      }
    }
  }
